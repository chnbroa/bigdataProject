...
data_name = ["검은별무늬병", "그을음병", "노균병", "덩굴마름병",
             "모자이크병", "세균성모무늬병", "탄저병", "흰가루병", "정상"]
...
for i in range(1, num_augmented_images):
    ...
    if(random_augment == 1):
        ...
        inverted_image.save(save_file_path + 'inverted_' +
                            str(augment_cnt) + '.png')
        ...
        rotated_image.save(save_file_path + 'rotated_' +
                           str(augment_cnt) + '.png')

    elif(random_augment == 3):
        # 노이즈 처리
        # 파일 cv2 한글처리? 추가
        img = cv2.imdecode(np.fromfile(origin_image_path,
        ...
        noisy_image.save(save_file_path + 'noiseAdded_' +
                         str(augment_cnt) + '.png')
